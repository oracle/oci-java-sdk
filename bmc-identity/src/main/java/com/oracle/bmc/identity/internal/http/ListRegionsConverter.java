...
public class ListRegionsConverter {
    ...
    public static Invocation.Builder fromRequest(RestClient client, ListRegionsRequest request) {
        if (request == null) {
            throw new NullPointerException("request instance is required");
        }

        WebTarget target = client.getBaseTarget().path("/20160918").path("regions");
    ...
    public static Function<Response, ListRegionsResponse> fromResponse() {
        final Function<Response, ListRegionsResponse> transformer =
                new Function<Response, ListRegionsResponse>() {
                    @Override
                    public ListRegionsResponse apply(Response rawResponse) {
                        LOG.trace("Transform function invoked for ListRegionsResponse");
                        Function<Response, WithHeaders<List<Region>>> responseFn =
                                RESPONSE_CONVERSION_FACTORY.create(
                                        new GenericType<List<Region>>() {});

                        WithHeaders<List<Region>> response = responseFn.apply(rawResponse);
