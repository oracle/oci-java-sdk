 ...
 * options are supported only for Exadata VM cluster on Exascale Infrastructure. isThinClone options
 * are supported only for Exadata VM cluster on Exascale Infrastructure. Use {@code
 ...
 * database (CDB) to create a remote clone. sourcePluggableDatabaseSnapshotId and isThinClone
 * options are supported only for Exadata VM cluster on Exascale Infrastructure. isThinClone options
 * are supported only for Exadata VM cluster on Exascale Infrastructure. Use {@code RELOCATE_PDB}
 * for relocating the Pluggable Database from Source CDB and creating it in target CDB. This will
 * relocate a pluggable database (PDB) to a different database from the source PDB. The source PDB
 * must be in the {@code READ_WRITE} openMode when performing the relocate. <br>
