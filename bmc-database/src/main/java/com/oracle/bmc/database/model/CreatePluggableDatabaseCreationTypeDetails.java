 ...
 * openMode to perform the clone operation. sourcePluggableDatabaseSnapshotId and isThinClone
 * options are supported for Exadata VM cluster on Exascale Infrastructure, Exadata Cloud@Customer
 * VM Cluster on Exadata Cloud@Customer infrastructure, and Exadata Cloud VM cluster on cloud
 * Exadata infrastructure Use {@code REMOTE_CLONE_PDB} for creating a new PDB using Remote Clone on
 * Source Pluggable Database. This will Clone a pluggable database (PDB) to a different database
 * from the source PDB. The cloned PDB will be started upon completion of the clone operation. The
 * source PDB must be in the {@code READ_WRITE} openMode when performing the clone. For Exadata
 * Cloud@Customer instances, the source pluggable database (PDB) must be on the same Exadata
 * Infrastructure as the target container database (CDB) to create a remote clone.
 * sourcePluggableDatabaseSnapshotId and isThinClone options are supported for Exadata VM cluster on
 * Exascale Infrastructure, Exadata Cloud@Customer VM Cluster on Exadata Cloud@Customer
