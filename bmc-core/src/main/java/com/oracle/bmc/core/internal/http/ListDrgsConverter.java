...
@Slf4j
public class ListDrgsConverter {
    ...
    public static Invocation.Builder fromRequest(RestClient client, ListDrgsRequest request) {
        ...
        WebTarget target = client.getBaseTarget().path("/20160918").path("drgs");
    ...
    public static Function<Response, ListDrgsResponse> fromResponse() {
        final Function<Response, ListDrgsResponse> transformer =
                new Function<Response, ListDrgsResponse>() {
                    @Override
                    public ListDrgsResponse apply(Response rawResponse) {
                        LOG.trace("Transform function invoked for ListDrgsResponse");
                        Function<Response, WithHeaders<List<Drg>>> responseFn =
                                RESPONSE_CONVERSION_FACTORY.create(new GenericType<List<Drg>>() {});

                        WithHeaders<List<Drg>> response = responseFn.apply(rawResponse);
