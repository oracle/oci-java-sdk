...
public class TestUserResource implements TestResource {
    // This retry policy is intended to be used when we try and ensure that a policy has been
    // applied
    // to the user created inside this resource. We retry on 404s as they'd indicate we're not
    // authorised
    // (or the compartment doesn't exist, but that shouldn't be the case based on where this policy
    // is used)
    ...
    /**
     * This makes sure that the policy applied to the temporary user in this class is active. The
     * policy we're giving them is:
     *
     * <p>Allow group <group> to manage all-resources on compartment <compartment>
     *
     * <p>So we can make sure that the policy is active by performing some action against the
     * compartment. In this case we'll create a policy in the compartment. This method will also
     * retry in case it takes longer to apply the policy than "normal" (according to the docs "New
     * policies take effect typically within 10 seconds")
     */
        ...
        try (Identity testClient =
            ...
            final String policyStatement =
                    String.format(
                            ...
                            policyTesterGroupName, compartmentName);
            ...
            // Try and create the policy. We don't have any waiters since a successful creation
            // should be sufficient to know that
