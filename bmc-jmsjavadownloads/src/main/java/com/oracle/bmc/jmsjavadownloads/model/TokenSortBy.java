...
public enum TokenSortBy implements com.oracle.bmc.http.internal.BmcEnum {
    ...
    private static java.util.Map<String, TokenSortBy> map;

    static {
        map = new java.util.HashMap<>();
        for (TokenSortBy v : TokenSortBy.values()) {
            map.put(v.getValue(), v);
        }
    }

    TokenSortBy(String value) {
        this.value = value;
    }
    ...
    public static TokenSortBy create(String key) {
        if (map.containsKey(key)) {
            return map.get(key);
        }
        throw new IllegalArgumentException("Invalid TokenSortBy: " + key);
    }
}
