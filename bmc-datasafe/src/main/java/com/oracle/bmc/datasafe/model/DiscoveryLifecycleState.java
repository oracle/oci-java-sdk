...
public enum DiscoveryLifecycleState {
    ...
    static {
        ...
        for (DiscoveryLifecycleState v : DiscoveryLifecycleState.values()) {
    ...
    DiscoveryLifecycleState(String value) {
        this.value = value;
    }
    ...
    public static DiscoveryLifecycleState create(String key) {
        if (map.containsKey(key)) {
            return map.get(key);
        }
        LOG.warn(
                "Received unknown value '{}' for enum 'DiscoveryLifecycleState', returning UnknownEnumValue",
                key);
