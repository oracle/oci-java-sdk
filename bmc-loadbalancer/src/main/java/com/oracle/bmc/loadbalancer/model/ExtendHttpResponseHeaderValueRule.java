...
/**
 * An object that represents the action of modifying a response header value. This rule applies only
 * to HTTP listeners.
 *
 * <p>This rule adds a prefix, a suffix, or both to the header value.
 *
 * <p>*NOTES:**
 *
 * <p>This rule requires a value for a prefix, suffix, or both.
 *
 * <p>The system does not support this rule for headers with multiple values.
 *
 * <p>The system does not distinquish between underscore and dash characters in headers. That is, it
 * treats {@code example_header_name} and {@code example-header-name} as identical. If two such
 ...
 * underscore or dash characters to uniquely distinguish header names. <br>
...
public final class ExtendHttpResponseHeaderValueRule extends Rule {
    ...
    public static class Builder {
        ...
        /**
         * A string to prepend to the header value. The resulting header value must still conform to
         * RFC 7230. With the following exceptions: * value cannot contain {@code $} * value cannot
         * contain patterns like {@code {variable_name}}. They are reserved for future extensions.
         * Currently, such values are invalid.
         *
         * <p>Example: {@code example_prefix_value}
         */
        ...
        /**
         * A string to append to the header value. The resulting header value must still conform to
         * RFC 7230. With the following exceptions: * value cannot contain {@code $} * value cannot
         * contain patterns like {@code {variable_name}}. They are reserved for future extensions.
         * Currently, such values are invalid.
         *
         * <p>Example: {@code example_suffix_value}
         */
    ...
    /**
     * A string to prepend to the header value. The resulting header value must still conform to RFC
     * 7230. With the following exceptions: * value cannot contain {@code $} * value cannot contain
     * patterns like {@code {variable_name}}. They are reserved for future extensions. Currently,
    ...
    /**
     * A string to append to the header value. The resulting header value must still conform to RFC
     * 7230. With the following exceptions: * value cannot contain {@code $} * value cannot contain
     * patterns like {@code {variable_name}}. They are reserved for future extensions. Currently,
     * such values are invalid.
     *
