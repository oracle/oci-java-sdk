...
public enum ManagementOption {
    ...
    private static final org.slf4j.Logger LOG =
            org.slf4j.LoggerFactory.getLogger(ManagementOption.class);

    private final String value;
    private static java.util.Map<String, ManagementOption> map;

    static {
        map = new java.util.HashMap<>();
        for (ManagementOption v : ManagementOption.values()) {
    ...
    public static ManagementOption create(String key) {
        if (map.containsKey(key)) {
            return map.get(key);
        }
        LOG.warn(
                "Received unknown value '{}' for enum 'ManagementOption', returning UnknownEnumValue",
                key);
