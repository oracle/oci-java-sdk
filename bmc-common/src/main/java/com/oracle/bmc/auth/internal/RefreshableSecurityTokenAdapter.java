...
/** An extension of @{@link SecurityTokenAdapter} that allows the security token to be refreshed **/
public class RefreshableSecurityTokenAdapter<T> {
    private static final Logger LOG =
            org.slf4j.LoggerFactory.getLogger(RefreshableSecurityTokenAdapter.class);
    private final T resourcePrincipalCreationConfig;
    private final java.util.function.Function<T, String> resourcePrincipalSessionTokenSupplier;

    private Optional<JwtClaimsSet> jwt;
    private Optional<RSAPublicKey> jwkAsJce;
    ...
    public RefreshableSecurityTokenAdapter(
            java.util.function.Function<T, String> resourcePrincipalSessionTokenSupplier,
            T resourcePrincipalCreationConfig,
            SessionKeySupplier sessionKeySupplier) {
        this(
                resourcePrincipalSessionTokenSupplier.apply(resourcePrincipalCreationConfig),
                sessionKeySupplier,
    ...
    public RefreshableSecurityTokenAdapter(
        ...
        this.resourcePrincipalSessionTokenSupplier = resourcePrincipalSessionTokenSupplier;
        this.resourcePrincipalCreationConfig = resourcePrincipalCreationConfig;
    }
    ...
    public void refreshToken() {
        LOG.info("Refreshing security token.");
        this.securityToken =
                resourcePrincipalSessionTokenSupplier.apply(resourcePrincipalCreationConfig);
        if (securityToken != null && !securityToken.isEmpty()) {
            JwtClaimsSet jwtClaims = new JwtClaimsSet(securityToken);
