...
public abstract class AbstractResponseIterator<REQUESTBUILDER, REQUEST, RESPONSE> {
     ...
     * the list operation so that the next page of results will be returned. This will just create
     * the request and not make any service calls
     *
     ...
     *
     * @return null if we have not previously made a request to fetch any results. Otherwise, an
     *     Optional containing the next page token to use. If there is no next page token to use
     *     then an empty/absent Optional will be returned
     */
