...
/**
 * A wrapper over an {@link java.io.FilterInputStream} which limits the reads of data from the
 * input stream up to a specified length that is shorter than the actual length of the stream.
 */
public class LengthLimitedInputStream extends FilterInputStream {
    ...
    public synchronized void mark(int ignoredReadlimit) {
        // mark() will cause the reset() to change the length to the stream's specified read limit length. Instead, it
        // should be the length of bytes that are yet to be read. The length should therefore match the remainingBytes.
