...
public class HttpEndpointAuthConfig extends com.oracle.bmc.http.internal.ExplicitlySetBmcModel {
    @Deprecated
     ...
     * - {@code HTTP_ENDPOINT_NO_AUTH_CONFIG}: Indicates that no authentication is required for invoking the endpoint.
     * - {@code HTTP_ENDPOINT_DELEGATED_BEARER_AUTH_CONFIG}: Specifies Bearer Token Authentication, where the same Bearer token received as part of the Agent Chat
     * API request is used to invoke the external endpoint.
     * - {@code HTTP_ENDPOINT_OCI_RESOURCE_PRINCIPAL_AUTH_CONFIG}: Specifies authentication using Oracle Cloud Infrastructure (OCI) Resource Principal, leveraging
     * OCI's identity-based authentication mechanism.
     * - {@code HTTP_ENDPOINT_IDCS_AUTH_CONFIG}: Specifies authentication using Oracle Identity Cloud Service (IDCS), leveraging OAuth 2.0 for token-based authentication.
     *
     **/
    public enum HttpEndpointAuthConfigType {
