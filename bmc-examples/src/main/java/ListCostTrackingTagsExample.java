...
public class ListCostTrackingTagsExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final TagNamespace tagNamespace1 = createTagNamespace(identityClient, tenantId, "SDK-Sample-NS", "Sample Tag Namespace in Tenancy");
        ...
        final GetTagNamespaceRequest getTagNamespaceRequest = GetTagNamespaceRequest.builder().tagNamespaceId(tagNamespace1.getId()).build();

        GetTagNamespaceResponse getTagNamespaceResponse = Failsafe.with(RETRY_POLICY)
                .get(
                    new Callable<GetTagNamespaceResponse>() {
                        public GetTagNamespaceResponse call() {
                            return identityClient.getTagNamespace(getTagNamespaceRequest);
                        }
                    });

        final Tag tag1 = createTag(identityClient, tagNamespace1.getId(), "SDK-Sample-CT-Tag", "Sample Cost Tracking Tag", true);

        // List all Cost Tracking Tags within tenancy
        System.out.println(
                "ListCostTrackingTags from Tenancy : " + tenantId + "\n");
        ...
        System.out.println(
                "ListCostTrackingTags Response for : " + tenantId + "\n");
    ...
    public static TagNamespace createTagNamespace (
        ...
        CreateTagNamespaceDetails createTagNamespaceDetails = CreateTagNamespaceDetails.builder()
        ...
        try {
            ...
            System.out.println("TagNamespace " + createTagNamespaceDetails.getName() + " created successfully");
            return tagNamespace;
        } catch (Exception ex) {
            throw new RuntimeException(
                    "TagNamespace creation fails with error:" + ex.getMessage());
        }
    }

    public static Tag createTag (
            Identity client, String tagNamespaceId, String name, String description, boolean isCostTracking) {

        CreateTagDetails createTagDetails = CreateTagDetails.builder()
        ...
        try {
            ...
            throw new RuntimeException(
                    "Tag creation fails with error:" + ex.getMessage());
        }
    }

    public static Tag updateTag (
            Identity client, String tagNamespaceId, String name, String description, boolean isCostTrackingTag) {

        UpdateTagDetails updateTagDetails = UpdateTagDetails.builder()
        ...
        try {
            ...
            if (tag == null) {
                throw new RuntimeException(
                        "Tag update fails with " + updateTagDetails.toString());
            }
            ...
            throw new RuntimeException(
                    "Tag update fails with error:" + ex.getMessage());
        }
    }
