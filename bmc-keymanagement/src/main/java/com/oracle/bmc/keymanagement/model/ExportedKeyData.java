...
/**
 * The response to a request to export key material. <br>
...
public final class ExportedKeyData
    ...
    public static class Builder {
        ...
        /**
         * The base64-encoded exported key material, which is encrypted by using the public RSA
         * wrapping key specified in the export request.
         */
         ...
         * and, therefore, cannot be exported). Specifying RSA_OAEP_AES_SHA256 invokes the RSA AES
         * key wrap mechanism, which generates a temporary AES key. The temporary AES key is wrapped
         * by the RSA public wrapping key provided along with the request, creating a wrapped
         * temporary AES key. The temporary AES key is also used to wrap the exportable key
         * material. The wrapped temporary AES key and the wrapped exportable key material are
         * concatenated, producing concatenated blob output that jointly represents them. Specifying
         * RSA_OAEP_SHA256 means that the exportable key material is wrapped by the RSA public
         * wrapping key provided along with the request.
         */
     ...
     * therefore, cannot be exported). Specifying RSA_OAEP_AES_SHA256 invokes the RSA AES key wrap
     * mechanism, which generates a temporary AES key. The temporary AES key is wrapped by the RSA
     * public wrapping key provided along with the request, creating a wrapped temporary AES key.
     * The temporary AES key is also used to wrap the exportable key material. The wrapped temporary
     * AES key and the wrapped exportable key material are concatenated, producing concatenated blob
     * output that jointly represents them. Specifying RSA_OAEP_SHA256 means that the exportable key
     * material is wrapped by the RSA public wrapping key provided along with the request.
     */
