...
/**
 * Allows returning dynamically generated HTTP response body based on the provided template. The
 * template allows variable interpolation by specifying variable name between the '${' and '}'
 * delimiters. Escape sequences using '\\' are supported to allow usage of '\\\\' and '\\${' in the
 * template to return '\\' and '\\${' in final response.
 *
 * <p>The following variables are supported:
 *
 * <p>http.request.id - the HTTP request ID. For example: "d5fa953f75ef417e4c8008ef9336d779".
 *
 * <p>Example: { "type": "DYNAMIC", "template": "{\ \\"code\\": 403,\
 * \\"message\\":\\"Unauthorised\\",\ \\"incidentId\\": \\"${http.request.id}\\"\ }" }
 *
 * <p>Example with escape sequence: { "type": "DYNAMIC", "template": "\\\\${Returned as plain text}"
 * } <br>
