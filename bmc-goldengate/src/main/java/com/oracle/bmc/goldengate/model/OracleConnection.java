...
public final class OracleConnection extends Connection {
    ...
    public static class Builder {
        ...
        /**
         * Specifies the session mode for the database connection. Use REDIRECT only for RAC
         * databases with SCAN listeners that return IP addresses. For RAC databases with SCAN
         * listeners that return FQDNs, and for all other Oracle database technologies, use DIRECT.
         * In RAC deployments, SCAN listeners redirects a connection to a specific database node,
         * identified by either IP address or FQDN. It is recommended to configure RAC with
         * FQDN-based SCAN listeners.
         *
    ...
    /**
     * Specifies the session mode for the database connection. Use REDIRECT only for RAC databases
     * with SCAN listeners that return IP addresses. For RAC databases with SCAN listeners that
     * return FQDNs, and for all other Oracle database technologies, use DIRECT. In RAC deployments,
     * SCAN listeners redirects a connection to a specific database node, identified by either IP
     * address or FQDN. It is recommended to configure RAC with FQDN-based SCAN listeners.
     *
     * <p>The default is DIRECT, except when databaseId is provided and the discovered database
     * relies on the SCAN listener. In this case, the default is REDIRECT.
     *
